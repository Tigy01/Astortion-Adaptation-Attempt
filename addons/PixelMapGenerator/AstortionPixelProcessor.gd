...
func process_assets(path: String) -> void:
	sprite_path = path
	...
	if (load(map_path) == null):
		print('Error Missing Map file')
		return
	var sprite_image:= _get_image(sprite_path)
	var map_image:= _get_image(map_path)
	
	var skin_colors: PackedColorArray = _make_color_array(sprite_image)
	var map_colors: PackedColorArray = _make_color_array(map_image)
	var map_coords: PackedVector2Array = _make_coord_array(map_image)
	
	_update_skin_colors(map_colors, map_coords, skin_colors)
	_finalize_and_export(sprite_image, skin_colors)
...
func _get_image(file_path: String) -> Image:
...
func _make_color_array(texture: Image) -> Array: 
	var color_data:= []
...
func _make_coord_array(texture: Image) -> Array: 
	var coord_data:= []
	for v in texture.get_size().y:
		for u in texture.get_size().x:
...
func _update_skin_colors(map_colors: Array, map_coords, skin_colors):
	for s in skin_colors.size():
		for m in map_colors.size(): 
			if skin_colors[s] == map_colors[m]: #compares the color of every pixel on the map to a given skin color
				skin_colors[s].r = map_coords[m].x / 255.0 #saves the color as the UV coordinate from the map and convers it to a 0-1 color format
				skin_colors[s].g = map_coords[m].y / 255.0
				skin_colors[s].b = 0


## Creates a new image the same size as the reference sprite sheet
func _finalize_and_export(sprite_image, skin_colors):
